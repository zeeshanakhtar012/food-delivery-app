...
exports.updateTable = async (req, res, next) => {
  try {
    ...
    // Dynamic update query builder could be used, but for simplicity:
    // ... skipping complex builder for now, assuming standard fields
    // Actually, let's just do a direct update for specific fields
    const { table_number, capacity, section, status, is_active } = updates;
...
exports.getTableActiveOrder = async (req, res, next) => {
  try {
    ...
    // Find order where table_id matches and status NOT delivered/cancelled
    // We need to assume your `orders` table has `table_id` column.
    // Wait, schema.sql (Step 666) DOES NOT show `table_id` in `orders`.
    // It shows `user_id`, `restaurant_id`, `rider_id`.
    // IF POS logic uses tables, we need `table_id` in `orders` too!
    // I should check `createDiningTablesTable.js`... wait, that just creates tables.
    // I need to ADD `table_id` to `orders` table as well!

    // Let's assume for now we return null, but I should fix the schema migration to add table_id to orders.
    // Actually, checking frontend `handlePlaceOrder` (Step 652):
    // table_id: orderType === 'dine_in' ? selectedTable : null
    // So the frontend SENDS table_id. The backend MUST store it.

    // I will add the column in the migration script.

    const sql = `
            SELECT * FROM orders 
            WHERE restaurant_id = $1 
            AND id IN (SELECT id FROM orders WHERE status NOT IN ('delivered', 'cancelled', 'picked_up'))
            -- AND table_id = $2 (Need to add this column first)
            LIMIT 1
        `;
    // Since I can't guarantee content of orders table yet, I'll return empty for now to avoid crash, 
    // OR implement a check if column exists.

    return successResponse(res, null, 'No active order (feature pending schema update)');
  } catch (error) {
    next(error);
  }
};