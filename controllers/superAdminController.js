// superAdminController.js (renamed and updated from adminController.js)
// Changes:
// - Renamed to superAdminController
// - Added toggleRestaurantStatus to manage restaurant isActive
// - Added deleteRestaurant to soft-delete restaurant and associated admin
// - Kept all other super admin functions
// - Discounts remain global (app-wide coupons managed by super admin)
// - Advertisements, app config, etc., stay here
...
// NEW: Toggle Restaurant Status (block/unblock restaurant)
const toggleRestaurantStatus = async (req, res) => {
  try {
    ...
    // Optionally, toggle associated admin status
    const restaurantAdmin = await AppUser.findOne({ restaurantId: id, role: 'restaurant_admin' });
    if (restaurantAdmin) {
      restaurantAdmin.isDeleted = !restaurant.isActive;
      restaurantAdmin.deletedAt = !restaurant.isActive ? new Date() : null;
      await restaurantAdmin.save();
    }
    ...
    await AuditLog.create({
      ...
      entity: 'Restaurant',
      entityId: id,
      details: `Restaurant ${restaurant.name} ${restaurant.isActive ? 'activated' : 'deactivated'}`,
      performedBy: req.user.userId,
    });

    console.log(`Restaurant ${restaurant.name} ${restaurant.isActive ? 'activated' : 'deactivated'}`);
    res.json({ message: `Restaurant ${restaurant.isActive ? 'activated' : 'deactivated'} successfully` });
  } catch (error) {
...
// NEW: Delete Restaurant (soft delete)
const deleteRestaurant = async (req, res) => {
  try {
    ...
    const restaurantAdmin = await AppUser.findOne({ restaurantId: id, role: 'restaurant_admin' });
    if (restaurantAdmin) {
      restaurantAdmin.isDeleted = true;
      restaurantAdmin.deletedAt = new Date();
      await restaurantAdmin.save();
    }
    ...
    await AuditLog.create({
      ...
      entity: 'Restaurant',
      entityId: id,
      details: `Restaurant ${restaurant.name} deleted`,
      performedBy: req.user.userId,
    });
...
// Create Discount (global, super admin only)
...
module.exports = {
  ...
  toggleRestaurantStatus, // NEW
  deleteRestaurant, // NEW
  updateAppConfig,
  getAppConfig,
  createAdvertisement,
