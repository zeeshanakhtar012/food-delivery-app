...
const createRestaurantWithAdmin = async (req, res) => {
  try {
    ...
    // Extract files
    const logo = req.files?.logo ? `/uploads/${req.files.logo[0].filename}` : 'https://example.com/default-logo.png';
    const profileImage = req.files?.profileImage ? `/uploads/${req.files.profileImage[0].filename}` : 'https://example.com/default-profile.png';

    // Validate restaurant data
    if (!name || !description || !street || !city || !state || !postalCode || !country || !phone || !email) {
      return res.status(400).json({ message: 'Restaurant name, description, address (street, city, state, postalCode, country), phone, and email are required' });
    }
